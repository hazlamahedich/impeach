...
// RED — Story 1.8 <Claim> primitive (UX-DR10, UX-DR36)
...
describe.skip('Story 1.8 — <Claim> variants (UX-DR10/UX-DR36)', () => {
  // RED — components/iip/claim absent
  ...
  it('REFUSES to render with zero citations (AC-2 — "no citation, no claim")', () => {
    // PD-1 essence: a Claim with zero citations is a build/runtime error, never served
    const { getByTestId, queryByText } = render(
      <Claim variant="fact" citations={[]}>Senator voted yes</Claim>,
    );
    // Renders nothing for the claim text (suppressed); shows insufficient badge
    expect(queryByText('Senator voted yes')).toBeNull();
    expect(getByTestId('trust-badge-insufficient')).toBeDefined();
  });
});
