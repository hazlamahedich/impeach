...
## Red-Phase Scaffolds
**File:** `tests/contract/fetch-adapter.contract.test.ts` (8 tests, all GREEN — activated Story 3.3)

- ✅ **[P0] FA-1:** the Crawler port is exported with discover() + fetch() + clean() — GREEN
- ✅ **[P0] FA-2:** FirecrawlAdapter (Tier-1) registered for scrapable sources — GREEN
- ✅ **[P1] FA-3:** ManualUploadAdapter (Tier-4) handles blocked sources — GREEN
- ✅ **[P0] FA-4:** two documents with same content_checksum deduplicated (processed once) — GREEN
- ✅ **[P1] FA-5:** two documents with different content_checksums both kept — GREEN
- ✅ **[P1] FA-6:** a manually uploaded document carries a full provenance record — GREEN
- ✅ **[P0] FA-7:** cleaned text is a faithful containment of the source (no OCR hallucination) — GREEN
- ✅ **[P1] FA-8:** FirecrawlAdapter.fetch() propagates an AbortSignal to the HTTP call — GREEN
...
| OCR fidelity (no hallucination) | FA-7 | RED |
| AbortSignal propagation | FA-8 | RED |
...
- [x] Create `packages/ingest/src/fetch/index.ts` exporting the `Crawler` port (abstract: discover, fetch, clean)
- [x] Create `packages/ingest/src/fetch/firecrawl.ts` — `FirecrawlAdapter implements Crawler` (Tier-1)
- [x] Create `packages/ingest/src/fetch/manual-upload.ts` — `ManualUploadAdapter` (Tier-4) carrying full provenance
- [x] Create `packages/ingest/src/fetch/registry.ts` — `adapterRegistry.byTier(n)` mapping
- [x] Create `packages/ingest/src/dedupe/dedup.ts` — `deduplicateDocuments(docs)` grouping by contentChecksum
- [x] Scaffold Tier-2 (Crawlee), Tier-3 (Alaveteli), Tier-5 (SFTP) as interfaces only (throw `NOT_IMPLEMENTED`)
- [x] Wire OCR (Docling+PaddleOCR-VL) into the PDF clean path (ADR-006) — separate integration test with real PDFs
- [x] Add `./fetch` to `packages/ingest` `package.json` `exports`
- [x] Remove `describe.skip` + convert dynamic import to direct import
- [x] Run `pnpm vitest --project contract -- fetch-adapter` → all 8 GREEN

## Tech-Debt (STORY-3.3-CLEANER-SPLIT)
- [ ] Extract `clean()` from `Crawler` port → separate `DocumentCleaner` port (per ADR-006 OcrPort precedent)
- [ ] Move FA-7 to new `cleaner.contract.test.ts`
- [ ] `ManualUploadAdapter` → implement `DocumentIngester` port (not `Crawler`) for `ingest()` method
- [ ] See tech-debt ticket for full acceptance criteria
