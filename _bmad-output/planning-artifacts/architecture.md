...
**Process boundaries (the AC-3-forced separation, STR-2/3):** six processes arbitrate via the OS scheduler on the single host — `api`, `ingest-worker` (sole AGE writer), `serve-worker` (read-path), `audit-worker` (append-only), `enqueuer` (control-plane), and `web` (Next.js 15 frontend). A runaway extraction loop cannot starve the query path. The Enqueuer is durable (Redis Streams), not an in-process EventEmitter.
...
- **Deploy:** single-host Docker Compose (transitional, AC-3); 6 processes arbitrate via OS scheduler (per ADR-021); all deps behind interfaces (SC-5) → multi-node = deployment change, not rewrite; Caddy ingress; isolated self-hosted runner (SEC-4).
