...
- **Apache AGE** — openCypher path (**SQL:PGQ does NOT exist in PG17/18, ADR-002**). Version pin pending verification (see open item above).
...
- **Polyglot eval bridge (SC-1, ADR-014):**
  ...
  - **`krippendorff` vs `simpledorff`:** pick ONE and pin — α implementations disagree on missing-data handling, which directly moves the inter-annotator agreement number.
...
- **No official Docker image exists with PG16 + pgvector + AGE + `pg_trgm` + `uuid-ossp` simultaneously.** Build a custom Dockerfile: `postgres:16` base → build AGE from source (C extension, PG dev headers, ~10–15 min) → build pgvector from source → enable extensions. **Pin the exact image digest, not a floating tag.** AGE is pre-1.0; "Trinity" release vs master is a fork risk.
...
- **`krippendorff` vs `simpledorff`:** pick ONE and pin — α implementations disagree on missing-data handling. **Floor: α ≥ 0.8 on safety-relevant labels** (Winston — conventional α ≥ 0.667 "tentative" is inadmissible for defamation-grade). **Solo team = theatre** — gate on real annotation volume or cut it.
