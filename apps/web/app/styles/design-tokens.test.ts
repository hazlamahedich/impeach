...
describe('Story 1.7 — Design token system (UX-DR1-8)', () => {
  ...
  it('dark-mode values differ from light-mode values for each token', () => {
    const css = readFileSync(TOKENS_PATH, 'utf8');
    ...
    for (const token of commonTokens) {
      expect(darkVars[token]).not.toBe(rootVars[token]);
    }
  });
  ...
  it('rounded scale tightened: 3/5/8/12 px (UX-DR6)', () => {
    const css = readFileSync(TOKENS_PATH, 'utf8');
