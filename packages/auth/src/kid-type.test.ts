/**
 * Kid type validation — non-string `kid` values must be rejected (SEC-1).
 *
 * @rules SEC-1
 */
...
describe('kid type validation', () => {
  it('rejects numeric kid in header before registry lookup', async () => {
    const verifyJwt = createVerifyJwt({
      keyRegistry: {
        get: (kid: string) => {
          if (kid === '123') throw new Error('registry should not be called with numeric kid');
          return undefined;
        },
